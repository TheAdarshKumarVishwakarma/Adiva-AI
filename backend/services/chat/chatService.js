...
router.post('/chat', optionalAuth, async (req, res) => {
  ...
  try {
    const { message, conversationId, systemPrompt, userPrompt, modelId, stream = false, regenerate = false } = req.body;
    ...
    if (!req.user) {
      ...
      const adminSettings = await chatRepository.getAdminSettings();
      ...
      const nextHistory = regenerate
        ? history
        : [...history, { role: 'user', content: message }, { role: 'assistant', content: aiResponse }];
      if (!regenerate) {
    ...
    // Get or create chat
    let chat = await chatRepository.findChatByConversationId(chatId, userId);
    ...
    if (!regenerate && adminSettings.settings.featureToggles?.analytics !== false) {
...
router.get('/user/chats', verifyToken, async (req, res) => {
  try {
    ...
    const totalChats = await chatRepository.countUserChats(userId, includeArchived);
...
router.get('/user/chats/:chatId', verifyToken, async (req, res) => {
  try {
    ...
    const chat = await chatRepository.findUserChatByIdWithUser(chatId, userId);
...
router.put('/user/chats/:chatId', verifyToken, async (req, res) => {
  try {
    ...
    const chat = await chatRepository.findUserChatById(chatId, userId);
...
router.get('/user/search', verifyToken, async (req, res) => {
  try {
    ...
    const chats = await chatRepository.searchUserChats(userId, q, {
